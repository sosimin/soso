...
public class BeginController {
	...
	public String Hello(Model model, @RequestParam(value="name", required=false)String name){
		...
		@RequestMapping("/bbb")//요청 url 중 /abc 경로가 표시 됐을때.
		...
		//http://localhost:7080/1701-spring/abc?name=gg
		// /abc가 경로임@@ return begin으로 치면 begin폴더에 begin.jsp로 실행하면됨 (파일명begin을 return하는거임) 
		@RequestMapping("/ff")//요청 url 중 /abc 경로가 표시 됐을때.
		//★★★ @RequestParam(value="name"이면 /ff?name=ㅎㅎㅎ 이렇게 쳐야함~~ ★★★
		public String showHello(Model model, @RequestParam(value="name", required=false)String name){
